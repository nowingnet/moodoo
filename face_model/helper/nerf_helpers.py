...
def dump_rays(origins, points, radiance_field):
    total_points = points.shape[0]*points.shape[1] #+ 1 origins.shape[0] +
    processed_radiance_field = torch.sigmoid(torch.relu(radiance_field[:,:,3]))
    threshold = 0.9999996
    indices = torch.where(processed_radiance_field > threshold)
    total_points = indices[0].shape[0]

    total_points=int(total_points//10)
    print("point cloud with %d points" %total_points)
    fid = open('rays_small.ply','w')
    fid.write('ply\n')
    ...
    for idx in tqdm(range(total_points)):
        ...
        ray_idx = indices[0][idx]
        depth_idx = indices[1][idx]
        color = radiance_field[ray_idx, depth_idx]
        fid.write("%f %f %f %d  %d %d\n" % (points[ray_idx, depth_idx, 0], points[ray_idx, depth_idx, 1], points[ray_idx, depth_idx, 2],
                                            color[0] * 255, color[1] * 255, color[2] * 255)
                  )
    # Write 3D points to .ply file
    # for i in range(points.shape[0]): # idx of ray
    #     for j in range(points.shape[1]): # idx of pt on ray
    #         if processed_radiance_field[i, j] <= 0.99999:
    #             continue
    #         color = radiance_field[i,j]
    #         #fid.write("%f %f %f %d  %d %d\n" % ( points[i,j,0],points[i,j,1],points[i,j,2], min(255,int((alphas[i,j])*255*3)) )
    #         fid.write("%f %f %f %d  %d %d\n" % ( points[i,j,0],points[i,j,1],points[i,j,2],
    #                                              color[0]*255, color[1]*255, color[2]*255 )
    #                   )
    #for i in range(origins.shape[0]):
    #    fid.write("%f %f %f 0 255 0\n" % (origins[i,0],origins[i,1],origins[i,2]))
    #fid.write("0 0 0 0 0 255")
